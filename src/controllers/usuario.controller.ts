...
export const importExcel: Handler = async (req, res) => {
	...
	if (files && Object.keys(files).length > 0 && files.file) {
		try {
			...
			const pathFile: string = await storeFile(<UploadedFile>files.file, 'usuarios', 'temp');
				...
				if (index >= rowStart) {
					...
					if (msg === 'ok') {
						...
						const newUsuario: IUsuario = new Usuario({
							...
							rol: rolPers?._id,
							departamento: usuario.departamento ? usuario.departamento?._id : req.body.departamento,
							anho: usuario.anho
						});

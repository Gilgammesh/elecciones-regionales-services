...
const nombre_submodulo: string = 'modulo'
const nombre_controlador: string = 'modulo.controller'
...
export const create: Handler = async (req, res) => {
  ...
  try {
    ...
    const moduloResp: IModulo | null = await Modulo.findById(
      moduloOut._id,
      exclude_campos
    )
    ...
    let msg: string = 'No se pudo crear el módulo'
...
export const update: Handler = async (req, res) => {
  ...
  try {
    ...
    // Inicializamos el mensaje de error
    let msg: string = 'No se pudo actualizar los datos del módulo'
    // Si existe un error con validación de campo único
    if (error?.errors) {
      // Obtenemos el array de errores
      const array: string[] = Object.keys(error.errors)

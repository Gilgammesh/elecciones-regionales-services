...
const sockets = (httpServer: HttpServer) => {
		...
		if (token && token !== '') {
			try {
				...
				if (error.name === 'NotBeforeError') {
					// Mostramos el error en consola
					console.log('Autenticando token Socket Middleware', 'NotBeforeError', error.message, error.date);
					// Generamos el error
	...
	io.on('connection', (socket: Socket) => {
		...
		socket.on('disconnect', () => {
			console.log('Cliente', socket.id, '->', 'desconectado', socket.disconnected);
		});
